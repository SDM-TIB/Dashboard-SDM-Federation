...
class MediatorPlanner(object):
    ...
    def includePhysicalOperatorsJoinBlock(self, jb):
        ...
        while len(tl) > 1:
            l = tl.pop(0)
            r = tl.pop(0)
            print("Calling physical operator join, from tl>1")
    ...
    def includePhysicalOperatorJoin(self, l, r):
        ...
        if isinstance(l, IndependentOperator) and l.tree.service.triples[0].subject.constant:
            if len(join_variables) > 0:
                n = TreePlan(NestedHashJoin(join_variables), all_variables, l, r, consts)
                dependent_join = True
        elif isinstance(r, IndependentOperator) and r.tree.service.triples[0].subject.constant:
            if len(join_variables) > 0:
                n = TreePlan(NestedHashJoin(join_variables), all_variables, r, l, consts)
                dependent_join = True
        elif isinstance(r, TreePlan) and r.operator.__class__.__name__ == "Xunion" and\
                isinstance(r.left, IndependentOperator) and \
                len(r.left.tree.service.triples) == 1 and\
                not r.left.tree.service.triples[0].subject.constant and\
                not r.left.tree.service.triples[0].subject.constant and\
                not r.left.tree.service.triples[0].predicate.constant and\
                isinstance(l, IndependentOperator):
            print(join_variables)
            n = TreePlan(NestedHashJoin(join_variables), all_variables, l, r, consts)
        elif isinstance(l, TreePlan) and l.operator.__class__.__name__ == "Xunion" and \
                isinstance(l.left, IndependentOperator) and \
                len(l.left.tree.service.triples) == 1 and\
                not l.left.tree.service.triples[0].subject.constant and \
                not l.left.tree.service.triples[0].predicate.constant and\
                not l.left.tree.service.triples[0].theobject.constant and \
                isinstance(r, IndependentOperator):
            n = TreePlan(NestedHashJoin(join_variables), all_variables, r, l, consts)
        elif isinstance(r, TreePlan) and r.operator.__class__.__name__ == "Xunion" and \
                isinstance(l, TreePlan) and l.operator.__class__.__name__ == "Xunion":
            # both are Union operators
            n = TreePlan(Xgjoin(join_variables), all_variables, l, r, consts)
            ...
            n = TreePlan(Xgjoin(join_variables), all_variables, r, l, consts)
...
class TreePlan(object):
    ...
    '''
    def __init__(self, operator, vars,  left=None, right=None,consts=set()):
...
class IndependentOperator(object):
    ...
    def execute(self, outputqueue, processqueue=Queue()):
        ...
        if self.tree.service.limit == -1:
            self.tree.service.limit = 100000 #TODO: Fixed value, this can be learnt in the future

        # Evaluate the independent operator.
        self.q = None
        self.q = Queue()
        # print(self.query_str, self.server, self.tree.service.limit)
