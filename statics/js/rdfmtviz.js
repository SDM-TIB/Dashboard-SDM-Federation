$(document).ready(function() {
    ...
    $.ajax({
                ...
                success: function(data, textStatus, jqXHR){
                    ...
                    $("a[class=fed]").click(function(){
                        fed = $(this).text()
                        $("#federationslistbtn").val($(this).text());
                        $("#fedName").html(fed);
                        $("#vfedName").html(fed);
                        $("#afedName").html(fed);
                        $("#datasources").empty();
    ...
    $("#mtdetails").click(function(){
        ...
        $.ajax({
                ...
                url: 'api/mtdetails?mt=' + url + "&fed="+federation,
                dataType: "json",
                crossDomain: true,
                success: function(data, textStatus, jqXHR){
                    console.log(url)
    ...
    function get_rdfmts_stats(fed){
        ...
        if (stats == null){
            ...
            stats.on( 'select', function ( e, dt, type, indexes ) {
                ...
                stats.clear().draw();
                stats.ajax.url("api/rdfmtstats?graph=" + fed).load();
            }
    }
    ...
    function get_rdfmts(fed){
        ...
        $.getJSON('api/rdfmts?graph=' + fed, function(data) {
           sources = data.sources;
           nodes = data.nodes;
           links = data.links;
...
//           });
        ...
        var circle = node.append("path")
              .attr("d", d3.svg.symbol()
                        .size(function(d) {
                            var v = d.size?Math.PI*Math.pow(size(85+d.size>200?200:d.size)||nominal_base_node_size,2):Math.PI*Math.pow(size(35)||nominal_base_node_size,2);
        ...
        if (text_center)
            ...
            text.attr("dx", function(d) {return (size(85)-size(30)||nominal_base_node_size);}) //size(d.weight)
        ...
        zoom.on("zoom", function() {
            ...
            if (nominal_base_node_size*zoom.scale()>max_base_node_size)
                ...
                circle.attr("d", d3.svg.symbol()
                        .size(function(d) {
                            var v = d.size?Math.PI*Math.pow(size(85+d.size>200?200:d.size)*base_radius/nominal_base_node_size||base_radius,2):Math.PI*Math.pow(size(35)*base_radius/nominal_base_node_size||base_radius,2);
                            return v;}) //size(d.weight)
                        .type(function(d) { return d.size? 'circle':  d.type; })
                   );
            ...
            if (!text_center)
                 text.attr("dx", function(d) {
                    return ((size(85)-size(30))*base_radius/nominal_base_node_size||base_radius);
                }); //size(d.weight)
    ...
    function get_rdfmts_graph_analys(fed, source){
        ...
        if (gtable == null){
            gtable = $('#graph-analysis').DataTable({
                ...
                ajax: 'api/rdfmtanalysis?graph='+fed+"&source="+source
            });
            galoaded = 1;
        }else {
            gtable.clear().draw();
            gtable.ajax.url("api/rdfmtanalysis?graph=" + fed+"&source="+source).load()
        }
    }
